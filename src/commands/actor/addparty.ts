...
import { onSuggestion as onSuggestionDefault, onSuggestionActive } from "./utils";
...
const onCommand = (handler: CommandHandler, args: string[]): void => {
    ...
    if ($gameParty.allMembers().length >= 4) {
        handler.log("Could not have more than 4 party members.", "red");
        return;
    }
...
const onSuggestion = (args: string[]): string[] => {
    ...
    const unique1 = array1.filter(value => array2.indexOf(value) === -1);
    const unique2 = array2.filter(value => array1.indexOf(value) === -1);
